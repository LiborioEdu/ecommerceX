...
export default function ProductDetail() {
  ...
  if (!product) return (
     <div className="min-h-screen bg-background-dark font-display text-white flex items-center justify-center">
         <div className="animate-pulse text-primary text-2xl uppercase tracking-widest font-black">Summoning Artifact...</div>
     </div>
  );
          ...
          <div className="lg:col-span-7 flex flex-col gap-6">
            <div className="relative aspect-square md:aspect-[4/5] rounded-xl overflow-hidden bg-[#1a1f26]/70 backdrop-blur-md border border-primary/20 shadow-[0_0_20px_rgba(245,159,10,0.15)]">
              <img 
                ...
                alt={product.name} 
                src={product.image || "https://placehold.co/600x800/1a1f26/f59e0b?text=Mythic+Item"}
              />
              ...
              <div className="absolute bottom-6 left-6 right-6 flex justify-between items-end">
                <div>
                  <span className="px-3 py-1 bg-primary/20 text-primary border border-primary/40 rounded-full text-xs font-bold uppercase tracking-widest mb-2 inline-block">
                    {product.category_name || "Mythic Grade"}
                  </span>
          ...
          <div className="lg:col-span-5 flex flex-col gap-8">
            <div>
              <div className="flex items-center gap-2 text-primary mb-2">
                ...
                <span className="text-slate-400 text-sm ml-2">(Ancient Reviews)</span>
              </div>
              <p className="text-slate-400 leading-relaxed mb-6">
                {product.description || "Forged from the remnants of a fallen star within the heart of Mount Ignis. Bestowed upon the heroes of the First Age, it now seeks a new master."}
              </p>
              
              <div className="flex items-baseline gap-4 mb-8">
                 <span className="text-4xl font-bold text-white">{Number(product.price).toFixed(0)} <span className="text-primary text-2xl">GP</span></span>
                 {product.price > 1000 && <span className="text-slate-500 line-through">{(product.price * 1.2).toFixed(0)} GP</span>}
              </div>
              ...
              <div className="grid grid-cols-3 gap-4 mb-8">
                <div className="bg-[#1a1f26]/70 backdrop-blur-md border border-primary/10 p-4 rounded-xl flex flex-col items-center text-center">
                    ...
                    <span className="text-xs text-slate-400 uppercase font-bold tracking-tighter">Attack</span>
                ...
                <div className="bg-[#1a1f26]/70 backdrop-blur-md border border-primary/10 p-4 rounded-xl flex flex-col items-center text-center">
                    ...
                    <span className="text-xs text-slate-400 uppercase font-bold tracking-tighter">Stock</span>
                ...
                <div className="bg-[#1a1f26]/70 backdrop-blur-md border border-primary/10 p-4 rounded-xl flex flex-col items-center text-center">
                    ...
                    <span className="text-xs text-slate-400 uppercase font-bold tracking-tighter">Rarity</span>
                    <span className="text-xl font-bold text-white uppercase tracking-wider text-sm mt-1">Epic</span>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-4">
                  ...
                  <Link to="/" className="w-full py-4 text-center rounded-xl border border-primary/30 text-primary font-bold uppercase tracking-widest hover:bg-primary/5 transition-all block">
                        Return to Armory
                  </Link>
              </div>
            </div>
